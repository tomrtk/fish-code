...
@core.post(
    "/projects/{project_id}/jobs/{job_id}/complete", response_model=schema.Job
)
def set_job_status_complete(
    project_id: int,
    job_id: int,
    repo: ProjectRepository = Depends(get_runtime_repo, use_cache=False),
):
    ...
    HTTPException
        If no job with _job_id_ found. Status code: 404.
    HTTPException
        If job is not currently running. Status code: 403.
    """
    project = repo.get(project_id)
    ...
    try:
        ...
        logger.warning("Cannot complete job %s, it's not running.", job_id)
        raise HTTPException(
            status_code=403, detail="Cannot complete job, it's not running."
        )
