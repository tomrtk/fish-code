...
class Progression:
    """Class to store progression in a job."""

    def __init__(self):
        self.videos: Dict[Video, int] = dict()
        self.total_frames = 0

    def _check_vid_tracked(self, vid: Video):
        if vid not in self.videos:
            logger.error(
                f"Video {vid._path} has not been added to the progression object."
            )
            raise RuntimeError

    def add(self, vid: Video):
        """Add a video to be tracked by progression."""
        if vid not in self.videos:
            self.videos[vid] = 0
            self.total_frames += vid.frames
        else:
            logger.error("Video {vid._path} is already tracked by progression.")
            raise RuntimeError

    def add_list(self, videos: List[Video]):
        """Add list of videos to be tracked by progression."""
        for vid in videos:
            self.add(vid)

    def is_video_tracked(self, vid: Video) -> bool:
        """Check if a video is tracked by the progression."""
        return vid in self.videos

    def start_batch(self, batchsize: int) -> int:
        """Get the start batch number given total progression."""
        if len(self.videos) == 0:
            raise RuntimeError(
                "Cannot get start batch number when no videos are tracked."
            )
        if self._total_processed == 0:
            return 0
        else:
            return math.ceil(self._total_processed / batchsize)

    def _frames_remaining_vid(self, vid: Video) -> int:
        """Get remaining frames to process in a video."""
        self._check_vid_tracked(vid)
        return vid.frames - self.videos[vid]

    @property
    def _total_processed(self) -> int:
        """Get the total amount of frames that have been processed."""
        if len(self.videos) > 0:
            return sum(self.videos.values())
        else:
            return 0

    def log_progress(self):
        """Log current progress to the info channel."""
        if len(self.videos) > 0 and self.total_frames > 0:
            percent = "{:.0%}".format(self._total_processed / self.total_frames)
            logger.info(
                f"Progress is {self._total_processed}/{self.total_frames} frames. ({percent})"
            )
        else:
            logger.info("Progress has no tracked videos to log.")

    def progress(self, vid: Video, frames: int):
        """Increment progress on a video by a number of frames."""
        self._check_vid_tracked(vid)
        if self.videos[vid] + frames > vid.frames:
            logger.error("Cannot progress past max frame of a video.")
            raise RuntimeError
        self.videos[vid] += frames
        self.log_progress()

    def is_processed(self, vid) -> bool:
        """Check if a video is processed."""
        self._check_vid_tracked(vid)
        return self.videos[vid] == vid.frames
